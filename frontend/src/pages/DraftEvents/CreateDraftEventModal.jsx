...
export default function CreateDraftEventModal({
  isOpen,
  onClose,
  ...
  async function checkDraftExist() {
    try {
      ...
      console.error("Error checking event group exist:", error);
      return false;
    }
  }
  ...
  async function handleCreateAccount(e) {
    ...
    const exists = await checkDraftExist();
    if (exists) {
      setErrorMsg(
        "event group with inputed name already exists. Make another one"
      );
      return;
    }
    try {
      ...
      const result = await res.json();
      if (result.status) {
        handleShowNotification("Event group created successfully.", "success");
        if (refreshData) refreshData();
        resetForm();
        onClose();
      } else {
        handleShowNotification(
          result.error || "Event group creation failed.",
          "error"
        );
      }
    } catch (error) {
      ...
      handleShowNotification(
        "Event group creation failed due to an error.",
        "error"
      );
    }
  }
          ...
          <div className="space-y-10 h-4/5 overflow-y-auto">
            <div className="mx-0.5">
              ...
              <Input
                ...
                placeholder="Enter the name of the draft event"
...
CreateDraftEventModal.propTypes = {
