...
export default function DeleteDraftBudgetModal({
  isOpen,
  onClose,
  onGoBack,
  id,
  ...
  async function handleDeleteBudget() {
    setLoading(true);
    try {
      const res = await fetch(`${ip}/delete-draft-budget`, {
        method: "POST",
      ...
      if (result.status) {
        handleShowNotification("Draft budget deleted successfully.", "success");
        if (refreshData) refreshData();
        handleOnClose();
        onClose();
      } else {
        handleShowNotification(
          result.error || "Draft budget deletion failed.",
          "error"
        );
      }
    } catch (error) {
      console.error("Error deleting draft budget:", error);
      handleShowNotification(
        "Draft budget deletion failed due to an error.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <div className="p-4 h-full w-full">
        <p className="h-1/2">
          Are you sure you want to permanently delete this draft budget? All
          related data, including budget breakdowns and payment details, will be
          permanently deleted.
        </p>
...
DeleteDraftBudgetModal.propTypes = {
