...
export default function CreateDraftBudgetModal({
  isOpen,
  onClose,
  ...
  async function checkBudgetExist() {
    try {
      const res = await fetch(`${ip}/check-budget-exist`, {
        method: "POST",
      ...
      console.error("Error checking budget exist:", error);
      return false;
    }
  }
  ...
  async function handleCreateAccount(e) {
    ...
    const exists = await checkBudgetExist();
    if (exists) {
      setErrorMsg("Budget with inputed name already exists. Make another one");
      return;
    }
    try {
      const res = await fetch(`${ip}/create-budget`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_data: userData,
      ...
      const result = await res.json();
      if (result.status) {
        handleShowNotification("Budget created successfully.", "success");
        if (refreshData) refreshData();
        resetForm();
        onClose();
      } else {
        handleShowNotification(
          result.error || "Budget creation failed.",
          "error"
        );
      }
    } catch (error) {
      ...
      handleShowNotification(
        "budget creation failed due to an error.",
        "error"
      );
    }
  }
          ...
          <div className="space-y-10 h-4/5 overflow-y-auto">
            <div className="mx-0.5">
              ...
              <Input
                ...
                placeholder="Enter the name of the draft budget"
...
CreateDraftBudgetModal.propTypes = {
