...
export default function CreateDraftPaymentModal({
  isOpen,
  onClose,
  ...
  async function checkPaymentExist() {
    try {
      const res = await fetch(`${ip}/check-payment-exist`, {
        method: "POST",
      ...
      console.error("Error checking payment exist:", error);
      return false;
    }
  }
  ...
  async function handleCreateAccount(e) {
    ...
    const exists = await checkPaymentExist();
    if (exists) {
      setErrorMsg("Payment with inputed name already exists. Make another one");
      return;
    }
    try {
      const res = await fetch(`${ip}/create-payment`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_data: userData,
      ...
      const result = await res.json();
      if (result.status) {
        handleShowNotification("Payment created successfully.", "success");
      ...
      handleShowNotification(
        "Payment creation failed due to an error.",
        "error"
      );
    }
  }
          ...
          <div className="space-y-10 h-4/5 overflow-y-auto">
            <div className="mx-0.5">
              ...
              <Input
                ...
                placeholder="Enter the name of the draft payment"
...
CreateDraftPaymentModal.propTypes = {
