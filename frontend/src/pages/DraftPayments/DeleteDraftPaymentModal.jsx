...
export default function DeleteDraftPaymentModal({
  isOpen,
  onClose,
  onGoBack,
  id,
  ...
  async function handleDeletepayment() {
    setLoading(true);
    try {
      const res = await fetch(`${ip}/delete-draft-payment`, {
        method: "POST",
      ...
      if (result.status) {
        handleShowNotification(
          "Draft payment deleted successfully.",
          "success"
        );
        ...
        handleShowNotification(
          result.error || "Draft payment deletion failed.",
          "error"
        );
      }
    } catch (error) {
      ...
      handleShowNotification(
        "Draft payment deletion failed due to an error.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <div className="p-4 h-full w-full">
        <p className="h-1/2">
          Are you sure you want to permanently delete this draft payment? All
...
DeleteDraftPaymentModal.propTypes = {
